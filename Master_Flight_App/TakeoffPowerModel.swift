...
struct TakeoffPower {
    ...
    mutating func calculatePower(airTemp: Int, antiIce: Bool, pressureAltitude: Int) -> (stringSHP100:String, stringSHP95: String) {
        ...
        //Limits the value of TempInput to 45 degrees
        if workingOAT > 45 {
            workingOAT = 45
        }
        ...
        // Pressure Altitude correction
         if pressureAltitude  >= 100 {
           SHP100 = SHP1077List[workingOAT - 17] - (12*(pressureAltitude/100))
           SHP95 = Double(SHP100) * 0.95
         }
         else {  //Calculated 100% & 95% SHP with no Pressure Altitude corrections

            SHP100 = SHP1077List[workingOAT - 17]
            SHP95 = Double(SHP100) * 0.95
        }
